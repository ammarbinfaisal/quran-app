// ---------------------------------------------------------------------------
// Mushaf
// ---------------------------------------------------------------------------
/** Supported mushaf layouts. v2 = KFGQPC V2 (604 pages, per-page QCF glyph
    font). i15 = Qudratullah Indopak 15-lines (610 pages, single shared
    Nastaleeq font, real Unicode Arabic text). */
export type MushafCode = "v2" | "i15";

export const MUSHAF_CODES: MushafCode[] = ["v2", "i15"];

/** Codes that use per-page QCF glyph fonts (PUA codepoints, not real text). */
export const QCF_CODES: MushafCode[] = ["v2"];

/** Codes that render real Unicode Arabic with a single shared font. */
export const UNICODE_CODES: MushafCode[] = ["i15"];

export const MUSHAF_DISPLAY_NAMES: Record<MushafCode, string> = {
  v2: "KFGQPC V2",
  i15: "Indopak (Qudratullah, 15-line)",
};

export const MUSHAF_LINES: Record<MushafCode, number> = {
  v2: 15,
  i15: 15,
};

/** Whether a mushaf registers one FontFace per page (QCF) or a single shared
    FontFace covering all pages (Indopak). */
export const MUSHAF_USES_PER_PAGE_FONT: Record<MushafCode, boolean> = {
  v2: true,
  i15: false,
};

// ---------------------------------------------------------------------------
// Mushaf page data (matches the JSON generated by scripts/generate-mushaf-assets.ts)
// ---------------------------------------------------------------------------

/** Row type in the mushaf layout. For v2 all rows are `ayah` (surah headers
    and basmallah are synthesised at render time from chapter metadata). For
    i15 the layout DB carries `surah_name` and `basmallah` rows explicitly. */
export type MushafLineType = "ayah" | "surah_name" | "basmallah";

export interface MushafWord {
  text: string;
  verseKey: string;
  x: number;
  width: number;
  charTypeName: string;
}

export interface MushafLine {
  lineNumber: number;
  /** Row type. Optional for backwards compatibility: v2 payloads generated
      before the Indopak support was added omit this and are treated as
      `"ayah"`. */
  lineType?: MushafLineType;
  centered?: boolean;
  /** Set on `surah_name` rows; the chapter whose header to render. */
  surahNumber?: number;
  words: MushafWord[];
}

export interface MushafPagePayload {
  page: number;
  mushafCode: string;
  lines: MushafLine[];
}

// ---------------------------------------------------------------------------
// Chapter metadata (from /chapters API)
// ---------------------------------------------------------------------------
export interface Chapter {
  id: number;
  nameSimple: string;
  nameArabic: string;
  versesCount: number;
  pages: [number, number]; // [startPage, endPage] in Madani mushaf
  revelationPlace: "makkah" | "madinah";
  bismillahPre: boolean;
}

// ---------------------------------------------------------------------------
// Translations
// ---------------------------------------------------------------------------
export type TranslationId = "saheeh" | "hilali-khan" | "abu-iyaad";
export const SUPPORTED_TRANSLATION_IDS = ["saheeh", "hilali-khan", "abu-iyaad"] as const satisfies readonly TranslationId[];

export type CopyVerseContentMode = "arabic" | "arabic-and-translations" | "translations";
export type AyahReciterId =
  | "maher-al-mu-aiqly"
  | "khalifa-al-tunaiji"
  | "saad-al-ghamdi"
  | "abdul-basit"
  | "husary"
  | "minshawi";
export type DataUsageMode = "low" | "balanced" | "high";

export const TRANSLATION_DISPLAY_NAMES: Record<TranslationId, string> = {
  saheeh: "Saheeh International",
  "hilali-khan": "Dr. Mohsin Khan and Shaykh Taqi-ud-Din al-Hilali",
  "abu-iyaad": "Shaykh Abu Iyaad",
};

export const AYAH_RECITER_DISPLAY_NAMES: Record<AyahReciterId, string> = {
  "maher-al-mu-aiqly": "Maher Al Mu'aiqly",
  "khalifa-al-tunaiji": "Khalifa Al Tunaiji",
  "saad-al-ghamdi": "Saad Al Ghamdi",
  "abdul-basit": "Abdul Basit Abdul Samad",
  husary: "Mahmoud Khalil al-Husary",
  minshawi: "Muhammad Siddiq al-Minshawi",
};

export const SUPPORTED_AYAH_RECITERS = [
  "maher-al-mu-aiqly",
  "khalifa-al-tunaiji",
  "saad-al-ghamdi",
  "abdul-basit",
  "husary",
  "minshawi",
] as const satisfies readonly AyahReciterId[];

/** Maps our internal translation ID to the api.quran.com resource ID */
export const TRANSLATION_API_IDS: Record<Exclude<TranslationId, "abu-iyaad">, number> = {
  saheeh: 20,
  "hilali-khan": 203,
};

export interface TranslationEntry {
  verseKey: string;
  text: string;
}

// ---------------------------------------------------------------------------
// Tafsir
// ---------------------------------------------------------------------------
export type TafsirId =
  | "ibn-katheer"
  | "tabari"
  | "qurtubi"
  | "baghawi"
  | "ibn-uthaymeen"
  | "ibn-alqayyim";

export const TAFSIR_IDS = [
  "ibn-katheer",
  "tabari",
  "qurtubi",
  "baghawi",
  "ibn-uthaymeen",
  "ibn-alqayyim",
] as const satisfies readonly TafsirId[];

export const TAFSIR_DISPLAY_NAMES: Record<TafsirId, string> = {
  "ibn-katheer": "Tafsir Ibn Kathir",
  tabari: "Tafsir al-Tabari",
  qurtubi: "Tafsir al-Qurtubi",
  baghawi: "Tafsir al-Baghawi",
  "ibn-uthaymeen": "Tafsir Ibn Uthaymeen",
  "ibn-alqayyim": "Tafsir Ibn al-Qayyim",
};

export const TAFSIR_ARABIC_NAMES: Record<TafsirId, string> = {
  "ibn-katheer": "تفسير ابن كثير",
  tabari: "تفسير الطبري",
  qurtubi: "تفسير القرطبي",
  baghawi: "تفسير البغوي",
  "ibn-uthaymeen": "تفسير ابن عثيمين",
  "ibn-alqayyim": "تفسير ابن القيم",
};

/** Whether the tafsir covers all 6236 ayaat (true) or only some (false) */
export const TAFSIR_FULL_COVERAGE: Record<TafsirId, boolean> = {
  "ibn-katheer": true,
  tabari: true,
  qurtubi: true,
  baghawi: true,
  "ibn-uthaymeen": false,
  "ibn-alqayyim": false,
};

// ---------------------------------------------------------------------------
// Themes
// ---------------------------------------------------------------------------
export type ThemeId =
  | "light-warm"
  | "dark-warm"
  | "white-green"
  | "classic-dark"
  | "blue-slate-dark"
  | "blue-slate-light";

export interface ThemeColors {
  bg: string;
  text: string;
  accent: string;
  surface: string;
  muted: string;
}

export const THEMES: Record<ThemeId, { label: string; colors: ThemeColors; pair?: ThemeId }> = {
  "light-warm": {
    label: "Warm Parchment",
    colors: { bg: "#f4ecd8", text: "#3c2e1c", accent: "#8a5a14", surface: "#eadfc3", muted: "#7e6947" },
    pair: "dark-warm",
  },
  "dark-warm": {
    label: "Lamplit Amber",
    colors: { bg: "#18120b", text: "#f0e0c2", accent: "#d7a44c", surface: "#261c11", muted: "#a08b65" },
    pair: "light-warm",
  },
  "white-green": {
    label: "Paper",
    colors: { bg: "#fafaf7", text: "#1a1614", accent: "#2d6a4f", surface: "#f0f0ea", muted: "#5c5a54" },
    pair: "classic-dark",
  },
  "classic-dark": {
    label: "Ink",
    colors: { bg: "#0d100e", text: "#e4e2db", accent: "#6aad84", surface: "#171a17", muted: "#86857c" },
    pair: "white-green",
  },
  "blue-slate-light": {
    label: "Cool Linen",
    colors: { bg: "#eef2f6", text: "#1f2a36", accent: "#1f6e6b", surface: "#e2e8ee", muted: "#54677a" },
    pair: "blue-slate-dark",
  },
  "blue-slate-dark": {
    label: "Deep Teal",
    colors: { bg: "#0e1820", text: "#d9e4ef", accent: "#5cb8b0", surface: "#182430", muted: "#7a8fa5" },
    pair: "blue-slate-light",
  },
};

// ---------------------------------------------------------------------------
// Reading mode
// ---------------------------------------------------------------------------
export type ReadingMode = "swipe" | "scroll";
export type ViewMode = "mushaf" | "vbv" | "scroll";
export type VbvSubmode = "s" | "j" | "p";

// ---------------------------------------------------------------------------
// User preferences (persisted to localStorage)
// ---------------------------------------------------------------------------
export interface UserPreferences {
  mushafCode: MushafCode;
  translationIds: TranslationId[];
  dataUsageMode: DataUsageMode;
  copyVerseContentMode: CopyVerseContentMode;
  copyTranslationIds: TranslationId[];
  ayahReciterId: AyahReciterId;
  inlineVerseNotes: boolean;
  theme: ThemeId;
  fontScale: number; // 1-10
  viewMode: ViewMode;
  vbvSubmode: VbvSubmode;
  scrollSubmode: VbvSubmode;
}

export const DEFAULT_PREFERENCES: UserPreferences = {
  mushafCode: "v2",
  translationIds: ["saheeh", "abu-iyaad"],
  dataUsageMode: "balanced",
  copyVerseContentMode: "arabic-and-translations",
  copyTranslationIds: ["saheeh", "abu-iyaad"],
  ayahReciterId: "maher-al-mu-aiqly",
  inlineVerseNotes: false,
  theme: "light-warm",
  fontScale: 1,
  viewMode: "mushaf",
  vbvSubmode: "s",
  scrollSubmode: "p",
};

// ---------------------------------------------------------------------------
// Reading history
// ---------------------------------------------------------------------------
export interface HistoryEntry {
  chapterId: number;
  chapterName: string;
  verseKey: string;
  pageNumber: number;
  timestamp: number;
}

// ---------------------------------------------------------------------------
// Offline / download status
// ---------------------------------------------------------------------------
export interface DownloadProgress {
  total: number;
  done: number;
  label: string;
}

export interface OfflineStatus {
  online: boolean;
  downloadedMushafs: MushafCode[];
  downloadedTranslations: TranslationId[];
}

// ---------------------------------------------------------------------------
// Audio
// ---------------------------------------------------------------------------
export interface AudioFile {
  chapterId: number;
  url: string;
  fileSize: number;
}
