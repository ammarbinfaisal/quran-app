...
export default async function IndopakVerseModeRoute({
  params,
}: {
  ...
  if (!/^(p|s|j)$/.test(type) || !/^[0-9]+$/.test(id)) {
    return (
      <InvalidPathMessage
        message={`"/iv/${type}/${id}" is not a valid view path.`}
      />
    );
  }
  ...
  return (
    <>
      <JsonLd
        id={`indopak-verse-mode-${validType}-${num}-jsonld`}
        data={createWebPageJsonLd({
          path: `/iv/${validType}/${num}`,
          title: `Indopak Verse Mode ${label} ${num}`,
          description: `Read Quran Indopak in verse mode by ${label.toLowerCase()} ${num}.`,
        })}
      />
      <Suspense>
        <VerseReader type={validType} id={num} forcedMushafCode="indopak" />
      </Suspense>
    </>
  );
}
