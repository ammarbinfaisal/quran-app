...
import { X, Play, Pause, SkipBack, SkipForward, Repeat, Loader2, Volume2 } from "lucide-react";
import {
  useRecitationPlayer,
  type RangeMode,
  type RecitationRange,
} from "@/components/recitation/useRecitationPlayer";
import { AYAH_RECITERS, type AyahReciterId } from "@/lib/ayahRecitation";
        ...
        <div className="px-4 py-5 space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2">
              {(Object.entries(AYAH_RECITERS) as [AyahReciterId, { label: string }][]).map(
                ([id, reciter]) => (
                  <button
                    key={id}
                    type="button"
                    onClick={() => setReciterId(id)}
                    className="flex items-center gap-3 rounded-lg border px-3 py-2.5 text-sm transition-colors text-left"
                    style={{
                      borderColor:
                        reciterId === id
                          ? "var(--color-accent)"
