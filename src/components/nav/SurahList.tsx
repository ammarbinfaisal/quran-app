...
export default function SurahList({ chapters, filter, onSelect }: SurahListProps) {
  ...
  const filtered = chapters.filter((ch) => {
    ...
    // Match against name
    if (ch.nameSimple.toLowerCase().includes(trimmed)) return true;
    // Match against translated name
    const translatedName = surahs?.[ch.id.toString()];
    if (translatedName && translatedName.toLowerCase().includes(trimmed)) return true;
    return false;
  });
        ...
        >
          ...
          <span className="flex flex-1 items-center justify-between gap-2 min-w-0">
            <span className="flex flex-col min-w-0">
              <span className="text-sm font-medium text-[var(--color-text)] truncate flex items-center gap-1.5 flex-wrap">
                <span>{ch.nameSimple}</span>
                {surahs?.[ch.id.toString()] && (
