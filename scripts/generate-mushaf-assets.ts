...
const TEXT_FIELD_BY_CODE: Record<MushafCode, keyof VerseWord> = {
  v1: "code_v1",
  v2: "code_v2",
  t4: "text_qpc_hafs",
  ut: "text_uthmani",
  ...
  i6: "text_indopak",
  qh: "text_qpc_hafs",
  tj: "text_uthmani_tajweed",
};

const LINE_FIELD_BY_CODE: Record<MushafCode, keyof VerseWord> = {
  v1: "line_v1",
  v2: "line_v2",
  t4: "line_number",
  ut: "line_number",
...
async function fetchVersesByPage(page: number): Promise<any[]> {
  const url =
    ...
    `?language=ar&words=true&per_page=50&word_fields=${encodeURIComponent(WORD_FIELDS)}`;
...
function generatePayload(
  ...
  for (const verse of verses) {
    const words = Array.isArray(verse.words) ? (verse.words as VerseWord[]) : [];
    for (const wordData of words) {
      // Keep both regular words and ayah-end markers/symbol tokens.
      // Restricting to only "word" drops end markers in several mushaf streams.
      if (
        wordData.char_type_name &&
        wordData.char_type_name !== "word" &&
        wordData.char_type_name !== "end"
      ) {
        continue;
      }
      ...
      const rawText = getWordText(wordData, textField, code);
      if (!rawText) continue;
...
function getWordText(
  word: VerseWord,
  preferredField: keyof VerseWord,
  code: MushafCode
): string {
  const preferredValue = normalizeWordText(word[preferredField]);
  if (preferredValue) return preferredValue;

  const fallbackCandidates: (keyof VerseWord)[] =
    code === "v1"
      ? ["code_v1", "text", "text_qpc_hafs", "text_uthmani", "text_indopak"]
      : code === "v2"
        ? ["code_v2", "text", "text_qpc_hafs", "text_uthmani", "text_indopak"]
        : code === "tj"
          ? ["text_uthmani_tajweed", "text_uthmani", "text_indopak"]
          : ["text_qpc_hafs", "text_uthmani", "text_indopak", "text"];
